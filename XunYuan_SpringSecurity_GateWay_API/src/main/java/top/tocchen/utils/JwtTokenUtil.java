...
public class JwtTokenUtil {
     ...
     * @return 布尔值
     */
    public boolean validateToken(String token, AdminUserDetails userDetails){
        UserAuthVO userAuthVO = userDetails.getUserAuthVO();
        // 通过三个方面进行验证 1. userId 2. Role 3. password
        HashMap<String, String> passMap = MD5Util.generateMD5(userAuthVO.getPassword(), userAuthVO.getSlat());
        String md5Password = passMap.get(MD5_STR_KEY);
        ...
        return userAuthVO.getUserId().equals(id)
                && userAuthVO.getRole() == role
                && md5Password.equals(password)
                && isTokenExpired(token)
                && userAuthVO.getUserName().equals(username);
    }
    ...
    public String generateToken(AdminUserDetails userDetails) {
        ...
        HashMap<String, String> passMap = MD5Util.generateMD5(userAuthVO.getPassword(), userAuthVO.getSlat());
        String md5Password = passMap.get(MD5_STR_KEY);
        claims.put(USER_INFO_ID, userAuthVO.getUserId());
        claims.put(USER_INFO_PASSWORD, md5Password);
        claims.put(USER_INFO_ROLE, userAuthVO.getRole());
        claims.put(USER_INFO_USERNAME, userAuthVO.getUserName());
