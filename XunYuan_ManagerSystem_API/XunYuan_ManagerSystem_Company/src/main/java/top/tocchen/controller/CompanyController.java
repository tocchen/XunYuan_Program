...
public class CompanyController {
    ...
    @ApiOperation(value = "通过id字段获取公司信息")
    @GetMapping("/query/{id}")
    public Response<?> queryCompanyById(@PathVariable("id") String id){
        CompanyEntity result = companyService.queryCompanyById(id);
        DBUtil.isEmpty2QueryException(result);
        return Response.success(result);
    }

    @GetMapping("/query/star/company")
    public Response<?> queryStarCompany(){
        List<CompanyEntity> result = companyService.queryStarCompany();
        return Response.success(result);
    }
}
