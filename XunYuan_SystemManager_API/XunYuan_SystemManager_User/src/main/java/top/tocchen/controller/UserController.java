...
import top.tocchen.service.impl.UserServiceImpl;
...
public class UserController {
    ...
    public Response<?> userLogin(@RequestBody JSONObject loginJOSN){
        UserEntity loginUser = loginJOSN.toJavaObject(UserEntity.class);
        if (!UserEntity.userBaseComplete(loginUser)) {
            throw new JSONFormatException();
        }

        UserEntity user = userService.queryMoreByUser(
    ...
    public Response<?> saveUser(@RequestBody JSONObject userJSON){
        UserEntity user = userJSON.toJavaObject(UserEntity.class);
        if (!UserEntity.userBaseComplete(user)) {
            throw new JSONFormatException();
        }
